...
using DTstrap.AppData;
using DTstrap.RobloxInterfaces;
using DTstrap.UI.Elements.Bootstrapper.Base;
...
namespace DTstrap
{
    public class Bootstrapper
    {
        ...
        public string BackgroundUpdaterMutexName => $"DTstrap-BackgroundUpdater-{_launchMode}";

        public string MutexNamePrefix { get; set; } = "DTstrap-Bootstrapper";
        ...
        private void HandleConnectionError(Exception exception)
        {
            ...
            if (exception is AggregateException aggEx && aggEx.InnerExceptions.Count > 0)
                exception = aggEx.InnerExceptions[0];
        ...
        public async Task Run()
        {
            ...
            // Extra guard: if BaseUrl is still null after connectivity check, something went wrong
            if (string.IsNullOrEmpty(Deployment.BaseUrl))
                HandleConnectionError(new Exception("Could not establish a connection to any Roblox server. Please check your internet connection."));
        ...
        private async Task GetLatestVersionInfo()
        {
            ...
            {
                SetStatus(Strings.Bootstrapper_Status_WaitingData);
            ...
            {
                if (string.IsNullOrEmpty(newVersionGuid))
                    throw new InvalidHTTPResponseException("Roblox returned an empty version GUID. The API may be temporarily unavailable.");
                ...
                if (string.IsNullOrEmpty(pkgManifestData))
                    throw new InvalidHTTPResponseException("Roblox returned an empty package manifest.");
        ...
        private async Task<bool> CheckForUpdates()
        {
            ...
            {
                App.Logger.WriteLine(LOG_IDENT, $"More than one DTstrap instance running, aborting update check");
                return false;
            }
            ...
            SetStatus(Strings.Bootstrapper_Status_UpgradingDTstrap);

            try
            {
#if DEBUG_UPDATER
                string downloadLocation = Path.Combine(Paths.TempUpdates, "DTstrap.exe");
        ...
        private async Task UpgradeRoblox()
        {
            ...
            if (Dialog is not null)
            {
                // Keep progress bar at 100% during extraction/configuring so it doesn't look frozen
                ...
                catch (Exception ex)
                {
                    ...
                    if (ex.GetType() == typeof(ChecksumFailedException))
                    {
                        ...
                        Frontend.ShowConnectivityDialog(
                            ...
                            String.Format(Strings.Dialog_Connectivity_UnableToDownloadReason, "[https://dtstraplabs.com/wiki/help/dtstrap-cannot-download-roblox/](https://dtstraplabs.com/wiki/help/dtstrap-cannot-download-roblox/)"),
