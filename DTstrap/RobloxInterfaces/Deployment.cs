namespace DTstrap.RobloxInterfaces
{
    public static class Deployment
    {
        ...
        public static async Task<Exception?> InitializeConnectivity()
        {
            ...
            while (tasks.Any() && String.IsNullOrEmpty(BaseUrl))
            {
                ...
                {
                    var ex = finishedTask.Exception!;
                    // AggregateException.InnerException can be null — use InnerExceptions instead
                    exceptions.Add(ex.InnerExceptions.Count > 0 ? ex.InnerExceptions[0] : ex);
                }
        ...
        {
            if (string.IsNullOrEmpty(BaseUrl))
                throw new InvalidOperationException("BaseUrl is not set — connectivity was not established before calling GetLocation.");
        ...
        public static async Task<ClientVersion> GetInfo(string? channel = null)
        {
            ...
            else
            {
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        App.Logger.WriteLine(LOG_IDENT, "Failed to contact clientsettings fallback too!");
                        App.Logger.WriteException(LOG_IDENT, ex2);
                        throw;
                    }
                }

                if (clientVersion is null || string.IsNullOrEmpty(clientVersion.VersionGuid))
                    throw new InvalidHTTPResponseException($"Received invalid or empty ClientVersion response from Roblox API");
